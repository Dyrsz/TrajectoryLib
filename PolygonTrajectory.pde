 ...
 class PolygonTrajectory extends Trajectory {
   ...
   private void AdjustPolygon (int nt, float cxt, float cyt, float rt, float anglet, float curvt) {
     ...
     for (int i = 0; i < n; i++) {
       ...
       if (curv == 0) {
         ...
         x2+lx2, y2+ly2, x3-x2, y3-y2, 0.01, false);
       }
       x1 = x2;
       y1 = y2;
     }
   ...
   float [] coord () {
     float [] ret = new float [2];
     tp = GetFunction ();
     ret [0] = cotr.coord (tp, GetSense ())[0];
     ret [1] = cotr.coord (tp, GetSense ())[1];
     return ret;
   }
   
   float [] coord (float tt) {
     float [] ret = new float [2];
     ret [0] = cotr.coord (tt, GetSense ())[0];
     ret [1] = cotr.coord (tt, GetSense ())[1];
     return ret;
   }
   
   float [] coord (float tt, boolean senset) {
     float [] ret = new float [2];
     ret [0] = cotr.coord (tt, senset)[0];
     ret [1] = cotr.coord (tt, senset)[1];
     return ret;
   }
   
   CompTrajectory GetTrajectory () {
     return cotr;
   }
